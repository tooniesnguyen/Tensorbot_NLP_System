...
if __name__ == "__main__":
    ...
    # Code xong bỏ thử có khác không
    ...
    with ptan.common.utils.TBMeanTracker(writer, 100) as tb_tracker:
        ...
        for epoch in range(MAX_EPOCHES):
            ...
            for batch in data.iterate_batches(train_data, BATCH_SIZE):
                ...
                # Đưa data embedd vào model encoder
                enc = net.encode(input_seq)
                ...
                for idx, inp_idx in enumerate(input_batch):
                    ...
                    # Nếu bleu lớn hơn 99% thì không cập nhật lại model => quay lại vòng for
                    if not args.disable_skip:
                        if argmax_bleu > 0.99:
                            skipped_samples += 1
                            continue

                    # Sau mỗi epoech sẽ hiển thị log
                    if not dial_shown:
                        # Giải mã input để hiển thị lên log infor input
                        w = data.decode_words(
                            inp_idx, rev_emb_dict)
                        log.info("Input: %s", utils.untokenize(w))

                        # Giải mã refer để hiển thị log
                        ref_words = [
                            utils.untokenize(
                                data.decode_words(
                                    ref, rev_emb_dict))
                    ...
                    # Để đánh giá model chọn random ổn định không thì ta sẽ tiến hành tính bleu từng câu    
                    for _ in range(args.samples):

                        # Lấy output của decode và action từ output đó suy luận ra
                        r_sample, actions =  net.decode_chain_sampling(
                                item_enc, beg_embedding,
                                data.MAX_TOKENS,
                                stop_at_token=end_token)
                        
                        # Tính toán bleu giữa action và ref
                        ...
                        # Append output của model, actions vào net_policie và action
                        net_policies.append(r_sample)
                        net_actions.extend(actions)
                        
                        # sample_bleu là câu dc tạo ra từ pp random còn argmax là câu dc tạo ra từ max, lấy các độ bleu đó
                ...
                # Nếu chưa có policies thì quay lại vòng lặp
                if not net_policies:
                    continue
